...
def show_guide():
    ...
    # Add instructions
    instructions = """
    Руководство по использованию LINKCHECK:
    ...
    Если в папке с linkcheck.exe уже присутствуют файлы ip.txt, com_table.xlsx, .env - то можно сразу начать проверку.
...
# UI Elements
label_welcome = ctk.CTkLabel(root, text="Добро пожаловать в LINKCHECK!", font=("Arial", 16, "bold"))
label_welcome.grid(row=0, column=0, columnspan=3, pady=2)
label_guide = ctk.CTkLabel(root, text="Заполните необходимые данные по порядку и нажмите на кнопку - Начать проверку")
label_guide.grid(row=1, column=0, columnspan=3, pady=2)

# Manual IP input
label_manual_ip = ctk.CTkLabel(root, text="Укажите IP адреса устройств:")
label_manual_ip.grid(row=2, column=0, columnspan=3, pady=2)
btn_manual_ip = ctk.CTkButton(root, text="Указать", command=manual_ip_input, width=200)
btn_manual_ip.grid(row=3, column=0, columnspan=3, pady=2)
checkmark_ip.grid(row=3, column=1, columnspan=3, pady=2)  # Place checkmark to the right of the button
...
label_download_sample.grid(row=4, column=0, columnspan=3, pady=2)
btn_download_sample = ctk.CTkButton(root, text="Скачать", command=download_sample_excel, width=200)
btn_download_sample.grid(row=5, column=0, columnspan=3, pady=2)
...
btn_upload_excel.grid(row=7, column=0, columnspan=3, pady=2)
checkmark_excel.grid(row=7, column=1, columnspan=3, pady=2)  # Place checkmark to the right of the button

# Login credentials
label_credentials = ctk.CTkLabel(root, text="Укажите логин и пароль:")
label_credentials.grid(row=8, column=0, columnspan=3, pady=2)
entry_username = ctk.CTkEntry(root, width=200)
entry_username.grid(row=9, column=0, columnspan=3, pady=2)
entry_username.insert(0, os.getenv("DEVICE_USERNAME", ""))
entry_password = ctk.CTkEntry(root, width=200, show="*")  # Hide password
entry_password.grid(row=10, column=0, columnspan=3, pady=2)
...
btn_save_credentials.grid(row=11, column=0, columnspan=3, pady=2)
checkmark_credentials.grid(row=11, column=1, columnspan=3, pady=2)  # Place checkmark to the right of the button

# Start button
btn_start_comparing = ctk.CTkButton(root, text="Начать проверку", command=start_comparing, width=200)
btn_start_comparing.grid(row=12, column=0, columnspan=3, pady=20)

# Configure grid weights to make the UI elements dynamically centralize
for i in range(9):
    root.grid_rowconfigure(i, weight=1, minsize=50)  # Adjust minsize for better scaling
for j in range(3):
    root.grid_columnconfigure(j, weight=1, minsize=50)
